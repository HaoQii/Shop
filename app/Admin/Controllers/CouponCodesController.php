...
class CouponCodesController extends AdminController
{
    ...
    /**
     * Make a form builder.
     *对于优惠码 code 字段，我们的第一个校验规则是 nullable，允许用户不填，不填的情况优惠码将由系统生成。对于折扣 value 字段，
     * 我们的校验规则是一个匿名函数，
     * 当我们的校验规则比较复杂，或者需要根据用户提交的其他字段来判断时就可以用匿名函数的方式来定义校验规则。
     * $form->saving() 方法用来注册一个事件处理器，在表单的数据被保存前会被触发，这里我们判断如果用户没有输入优惠码，
     * 就通过 findAvailableCode() 来自动生成。
     * @return Form
     */
